  ...
  const updateContent = (comments: Comments[]): Comments[] => {
    return comments.map((comment) => {
      if (comment.id === payload.id) {
        return {
          ...comment,
          replies: [...(comment.replies || []), payload.reply], // Ensure replies is initialized as an array
        };
      }
      if (comment.replies && comment.replies.length > 0) {
        return {
  ...
  const updateContent = (comments: Comments[]): Comments[] => {
    return comments.map((comment) => {
      if (comment.id === payload.id) {
        return {
          ...comment,
          score: comment.score > 0 ? comment.score - 1 : comment.score,
        };
      }
      if (comment.replies && comment.replies.length > 0) {
        return {
  ...
  const deleteContent = (comments: Comments[]): Comments[] => {
    return comments.filter((comment) => {
      if (comment.id === payload.id) {
        ...
        comment.replies = deleteContent(comment.replies as Comments[]);
      }
