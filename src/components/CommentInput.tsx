...
const CommentInput = () => {
  return (
    <div className="w-full py-6 px-4 bg-gray-200 shadow-sm rounded-md grid comment-input gap-x-3">
        <img src={amyrobsonImg} alt="avatar icon" className = "w-10 h-10 rounded-full max-sm:row-start-2 max-sm:row-span-1 col-start-1 col-span-1" />
        <input 
            type="text" 
        ...
        />
        <button type="button" className="py-4 w-24 h-fit bg-blue text-size-500 font-medium text-white rounded-lg uppercase justify-self-end max-sm:row-start-2 max-sm:row-span-1 col-start-3 col-span-1">Send</button>
    </div>
  )
}
