...
const CommentSection = () => {
  ...
  const sortedComment:Comments[] = [...comments].sort((a:Comments, b:Comments) => b.score - a.score);

  const commentList = sortedComment.map((comment:Comments)=> (
    <li className="w-full h-fit" key = {comment.id}>
        <CommentList setIsDeleting = {setIsDeleting} comment={comment} currentUser={currentUser} />
        <ul className="h-fit flex flex-col gap-y-4 items-end">
          { //the code ([...comment.replies].sort((a:Replies, b:Replies) => b.score - a.score)) is used to order the score for the replies too
              comment.replies.length > 0 && ([...comment.replies].sort((a:Replies, b:Replies) => b.score - a.score)).map((reply:Replies) => {
              return (
                <li className="w-[90%] h-fit first:mt-4" key = {reply.id}>
                  <ReplyList reply={reply} currentReply = {reply} currentUser={currentUser} setIsDeleting = {setIsDeleting} />
                  <ul className="h-fit flex flex-col gap-y-4 items-end">
                    { //the code ([...reply.replies].sort((a:Replies, b:Replies) => b.score - a.score)) is used to order the score for the replies too
                      reply.replies && reply.replies.length > 0 && ([...reply.replies].sort((a:Replies, b:Replies) => b.score - a.score)).map((reply:Replies) => {
                      return (
                        <li className="w-[90%] h-fit first:mt-4" key = {reply.id}>
                            <ReplyList reply={reply} currentUser={currentUser} setIsDeleting = {setIsDeleting} />
                        </li>
                      )}
                    )}
                    </ul>
  ...
  return (
    <section className = "w-full min-h-screen border-2 border-black bg-gray-100 grid gap-y-5 place-items-center py-6">
      ...
      <Modal open={isDeleting.status} onClose={() => setIsDeleting({...isDeleting, status: false})}>
        <div className="w-[20rem] p-6">
          <div className="w-full mb-4">
            <h3 className="text-size-500 font-black text-black mb-2">Delete comment</h3>
            <p className="text-size-400 text-gray-blue-100 font-medium">
              Are you sure you want to delete this comment? this will remove this comment and it would be undone.
            </p>
          </div>
